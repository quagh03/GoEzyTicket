...
Owns Postgres schema `identity`, including its own `identity.flyway_schema_history`. One Flyway instance per
...
history tables are in their own schemas, so `identity`'s `V1` and another module's `V1` never meet.
...
- Fully qualify objects with the schema: `CREATE TABLE identity.foo (...)`. Flyway sets `search_path`
  to `identity`, but being explicit survives copy-paste into psql.
