...
Owns Postgres schema `inventory`, including its own `inventory.flyway_schema_history`. One Flyway instance per
...
history tables are in their own schemas, so `inventory`'s `V1` and another module's `V1` never meet.
...
- Fully qualify objects with the schema: `CREATE TABLE inventory.foo (...)`. Flyway sets `search_path`
  to `inventory`, but being explicit survives copy-paste into psql.
