...
Owns Postgres schema `ticketing`, including its own `ticketing.flyway_schema_history`. One Flyway instance per
...
history tables are in their own schemas, so `ticketing`'s `V1` and another module's `V1` never meet.
...
- Fully qualify objects with the schema: `CREATE TABLE ticketing.foo (...)`. Flyway sets `search_path`
  to `ticketing`, but being explicit survives copy-paste into psql.
