...
Owns Postgres schema `ordering`, including its own `ordering.flyway_schema_history`. One Flyway instance per
...
history tables are in their own schemas, so `ordering`'s `V1` and another module's `V1` never meet.
...
- Fully qualify objects with the schema: `CREATE TABLE ordering.foo (...)`. Flyway sets `search_path`
  to `ordering`, but being explicit survives copy-paste into psql.
