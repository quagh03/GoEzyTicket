...
Owns Postgres schema `catalog`, including its own `catalog.flyway_schema_history`. One Flyway instance per
...
history tables are in their own schemas, so `catalog`'s `V1` and another module's `V1` never meet.
...
- Fully qualify objects with the schema: `CREATE TABLE catalog.foo (...)`. Flyway sets `search_path`
  to `catalog`, but being explicit survives copy-paste into psql.
