...
Owns Postgres schema `payment`, including its own `payment.flyway_schema_history`. One Flyway instance per
...
history tables are in their own schemas, so `payment`'s `V1` and another module's `V1` never meet.
...
- Fully qualify objects with the schema: `CREATE TABLE payment.foo (...)`. Flyway sets `search_path`
  to `payment`, but being explicit survives copy-paste into psql.
